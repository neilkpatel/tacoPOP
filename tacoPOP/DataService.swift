...
//implement dataservice, then talk about delegate pattern

protocol DataServiceDelegate: class {
    func deliciousTacoDataLoaded() //the one requirement of protocol
}


class DataService { //reason using a class is because this is going to be a singleton. Singleton will be one instance that lives throughout whole app, so you can refer to DataService from anywhere. Perfect kind of thing for singleton
    static let instance = DataService()
    
    weak var delegate: DataServiceDelegate? //may or may not have one
    
    var tacoArray: Array<Taco> = []
    
    func loadDeliciousTacoData() {
//appending to taco array one taco at a time. 
        ...
        delegate?.deliciousTacoDataLoaded() //any code that conforms to dataservicedelegate protocl, function will be fired and it will be notified.
